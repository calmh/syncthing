...
func (m *model) Index(conn protocol.Connection, idx *protocol.Index) error {
	return m.handleIndex(conn, idx.Folder, idx.Files, false, idx.PrevSequence, idx.MaxSequence)
}

// IndexUpdate is called for incremental updates to connected devices' indexes.
// Implements the protocol.Model interface.
func (m *model) IndexUpdate(conn protocol.Connection, idxUp *protocol.IndexUpdate) error {
	return m.handleIndex(conn, idxUp.Folder, idxUp.Files, true, idxUp.PrevSequence, idxUp.MaxSequence)
}

func (m *model) handleIndex(conn protocol.Connection, folder string, fs []protocol.FileInfo, update bool, prevSeq, maxSeq int64) error {
	op := "Index"
	if update {
		op += " update"
	}
	...
	return indexHandler.ReceiveIndex(folder, fs, update, op, prevSeq, maxSeq)
}
